...
# Modern smolagents (2025/2026) moved these to 'models'
try:
    ...
    try:
        ...
        # If it's the very latest version, it might be InferenceClientModel
        from smolagents import InferenceClientModel as HfApiModel

try:
    from smolagents import OpenAIServerModel, TransformersModel
except ImportError:
    from smolagents.models import OpenAIServerModel, TransformersModel

print(f"--- SAGA DEBUG: Using smolagents version {smolagents.__version__} ---")
...
class AgentWrapper(LocalAgent):
    ...
    def _initialize_base_model(self):
        if self.config.model_type == "huggingface":
            self.model = HfApiModel(model_id=self.config.model)
            return
        elif self.config.model_type == "TransformersModel":
            ...
            API_KEY = os.getenv(
                "OPENAI_API_KEY", None) if "https://api.openai.com" in self.config.api_base else ""
            if API_KEY is None:
                raise ValueError(
                    "OPENAI_API_KEY environment variable must be set for OpenAIServerModel.")
    ...
    def _email_tools(self):
        ...
        def check_inbox(limit: int = 50) -> List[dict]:
            ...
            """
            inbox_emails = self.email_client.get_emails(
                where="inbox", limit=limit)
            return inbox_emails
    ...
    def _calendar_tools(self):
        ...
        def get_free_time_slots(time_from: str, time_to: str) -> Tuple[bool, List[dict]]:
            ...
            """
            available_slots = self.calendar_client.get_availability(
                time_from, time_to)
    ...
    def _documents_tools(self):
        ...
        self.documents_client = LocalDocumentsTool(
            user_email=self.user_config.email)
    ...
    def _create_local_agent_object(self, **kwargs) -> MultiStepAgent:
        raise NotImplementedError(
            "Child class should implement _create_local_agent_object()")
    ...
    def _initialize_agent(self, initiating_agent: bool, task: str = None) -> MultiStepAgent:
        ...
        # Add preamble
        template_text = self.custom_prompt["system_prompt"].replace(
            "[[[preamble]]]", preamble)
        # Add task finished token
        template_text = template_text.replace(
            "[[[task_finished_token]]]", self.task_finished_token)
        # Fill in today's date
        today_date = datetime.now().strftime("%A, %B %d, %Y")
        template_text = template_text.replace("[[[today_date]]]", today_date)
        # Also replace to provide custom agent instructions
        template_text = template_text.replace(
            "[[[specific_agent_instruction]]]", self.config.specific_agent_instruction)
    ...
    def run(self, query: str,
        ...
        if agent_instance is None:
            ...
            agent_instance = self._initialize_agent(
                initiating_agent, task=task_str)

        # Make sure kwargs do not specify reset (should be False)
        if "reset" in kwargs:
            print(
                "WARNING: 'reset'' should not be specified in kwargs to agent, as it is always False.")
            kwargs.pop("reset")
...
class CodeAgentWrapper(AgentWrapper):
    ...
    def _create_local_agent_object(self, **kwargs) -> CodeAgent:
        ...
        starting_prompt_template = yaml.safe_load(
            importlib.resources.files("smolagents.prompts").joinpath(
                "code_agent.yaml").read_text()
        )
...
def get_agent(user_config: UserConfig,
    ...
    if config.base_agent_type == "CodeAgent":
        ...
        raise NotImplementedError(
            "ToolCallingAgent is not implemented yet. Use CodeAgentWrapper instead.")

    raise NotImplementedError(
        f"Base agent type {config.base_agent_type} not supported.")
