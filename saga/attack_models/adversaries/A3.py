...
class A3:
    # =======================================================================
    # ADVERSARIAL AGENT 3: An adversarial agent contacts the victim agent 
    # with the TLS public keys, but expired access control token. In this 
    # case, the victim agent establishes the TLS connection, but should not 
    # allow subsequent interaction with the adversarial agent.
    # 
    # This behavior is simulated by having the initiating agent not discard
    # previously used tokens once the quota is reached or when they expire. 
    # The agent is going to open a connection again with the receiving agent
    # using a no longer valid token.
    # =======================================================================
    ...
    def get_provider_cert(self):
        ...
        response = requests.get(PROVIDER_ENDPOINT+"/certificate", verify=False, cert=(
    ...
    def lookup(self, t_aid):
        response = requests.post(f"{saga.config.PROVIDER_CONFIG.get('endpoint')}/lookup", json={'t_aid': t_aid}, verify=False, cert=(
            self.workdir+"agent.crt", self.workdir+"agent.key"
        )) 
    ...
    def access(self, t_aid):
        response = requests.post(f"{saga.config.PROVIDER_CONFIG.get('endpoint')}/access", json={'i_aid':self.aid, 't_aid': t_aid}, verify=False, cert=(
            self.workdir+"agent.crt", self.workdir+"agent.key"
        )) 
        if response.status_code == 200:
            data = response.json()
