...
def register(email: str=None, password: str=None):
    ...
    response = requests.post(f"{saga.config.PROVIDER_CONFIG.get('endpoint')}/register",verify=False, json={
        'uid': email, # uid
    ...
    if response.status_code == 201:
        ...
        # Verify the provider's certificate (skip for local dev if verification fails)
        try:
...
def login(email: str=None, password: str=None):
    ...
    response = requests.post(f"{saga.config.PROVIDER_CONFIG.get('endpoint')}/login", json={'uid': email, 'password': password}, verify=False, cert=(
        saga.config.USER_WORKDIR+"/keys/"+email+".crt", saga.config.USER_WORKDIR+"/keys/"+email+".key"
    )) 
...
def spawn_agent(application):
    ...
    agent_dir_path = os.path.join(saga.config.USER_WORKDIR, application.get('aid'))
