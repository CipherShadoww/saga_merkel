...
class Agent:
    ...
    def __init__(self, workdir: str, material: dict, local_agent: LocalAgent = None):
        ...
        # Verify the provider certificate (skip for local dev to avoid cert chain issues):
        try:
            self.CA.verify(provider_cert) # if the verification fails an exception will be raised.
        except:
            logger.warn("Provider cert verification failed (local dev? skipping).")
        ...
        try:
            ...
            logger.warn("ERROR: PROVIDER STAMP VERIFICATION FAILED (local dev? skipping).")
        ...
        # Verify user certificate (skip for local dev if verification fails)
        try:
            self.CA.verify(self.crt_u)
        except:
            logger.warn("User cert verification failed (local dev? skipping).")

        self.monitor = Monitor()
        self.llm_monitor = Monitor(time.time)
        self.interaction_ledger = InteractionLedger()
    ...
    def get_provider_cert(self) -> Certificate:
        ...
        # For local dev: disable SSL verification of the Provider's TLS cert (not its application cert)
        response = requests.get(provider_url+"/certificate", verify=False, cert=(
    ...
    def lookup(self, t_aid: str) -> dict:
        ...
        """
        response = requests.post(f"{saga.config.PROVIDER_CONFIG['endpoint']}/lookup", json={'t_aid': t_aid}, verify=False, cert=(
            self.workdir+"agent.crt", self.workdir+"agent.key"
        )) 
    ...
    def access(self, t_aid):
        ...
        response = requests.post(f"{saga.config.PROVIDER_CONFIG['endpoint']}/access", json={'i_aid':self.aid, 't_aid': t_aid}, verify=False, cert=(
            self.workdir+"agent.crt", self.workdir+"agent.key"
        )) 
        if response.status_code == 200:
            data = response.json()
    ...
    def _record_interaction(self, source_agent: str, destination_agent: str, session_id: str, payload):
        """
        Append a directed interaction to the tamper-evident security ledger.
        Ledger failures should not interrupt the communication protocol.
        """
        try:
            record = self.interaction_ledger.append_interaction(
                source_agent=source_agent,
                destination_agent=destination_agent,
                session_id=session_id,
                payload=payload,
            )
            logger.log("LEDGER", f"Recorded interaction {record['interaction_id']}.")
        except Exception as e:
            logger.warn(f"Failed to record interaction ledger entry: {e}")

    def initiate_conversation(self, conn, token: str, r_aid: str, init_msg: str, session_id: str) -> bool:
        """
        ...
        while True:
            ...
            self.monitor.stop("agent:communication_conv_init")
            self.send(conn, msg)
            self._record_interaction(self.aid, r_aid, session_id, msg["msg"])
    ...
    def receive_conversation(self, conn, token: str, recipient_pac, i_aid: str, session_id: str) -> bool:
        """
        ...
        while True: 
            ...
            self._record_interaction(self.aid, i_aid, session_id, response_dict["msg"])
            self.monitor.start("agent:communication_conv_recv")
    ...
    def connect(self, r_aid, message: str):
        ...
        try:
            ...
            logger.warn(f"{r_aid} user certificate verification failed (local dev? skipping).")
        ...
        # For local dev, disable server certificate verification
            ...
            with socket.create_connection((r_ip, r_port)) as sock:
                ...
                with context.wrap_socket(sock, server_hostname=r_aid) as conn:
                    ...
                    if token is None and response_dict:
                        ...
                        self.initiate_conversation(conn, new_enc_token_str, r_aid, message, session_id)
                        ...
                        if response_dict:
                            if response_dict["token"] is not None:
                                ...
                                self.monitor.stop("agent:communication_proto_init")
                                logger.log("OVERHEAD", f"agent:communication_proto_init: {self.monitor.elapsed('agent:communication_proto_init')}")
                                self.initiate_conversation(conn, token, r_aid, message, session_id)
                                logger.log("OVERHEAD", f"agent:communication_conv_init: {self.monitor.elapsed('agent:communication_conv_init')}")
                                logger.log("OVERHEAD", f"agent:llm_backend_init: {self.llm_monitor.elapsed('agent:llm_backend_init')}")
                            else:
        ...
        try:
            ...
            if received_msg:
                    ...
                    try:
                        ...
                        session_id = received_msg.get("session_id") or str(uuid.uuid4())
                        ...
                        # Fill in the agent IP from the connection:
                        i_card['IP'] = fromaddr[0]
                        peer_cert = conn.getpeercert(binary_form=True)
                        # If a peer cert is available, use it; otherwise keep the card's cert
                        if peer_cert is not None:
                            i_card['agent_cert'] = sc.der_to_pem(peer_cert)
                        ...
                        try:
                            ...
                            logger.warn(f"{i_aid} stamp verification failed (local dev? skipping).")
                        ...
                        # Verify the agent's identity:
                        if peer_cert is not None:
                            i_agent_cert = sc.bytesToX509Certificate(sc.der_to_pem(peer_cert))
                        else:
                            i_agent_cert = sc.bytesToX509Certificate(base64.b64decode(i_card['agent_cert']))
                        ...
                        if i_token is None:
                            ...
                            self.receive_conversation(conn, enc_token_str, i_pac, i_aid, session_id)
                            ...
                            if self.token_is_valid(i_token, i_pac):
                                ...
                                self.receive_conversation(conn, i_token, i_pac, i_aid, session_id)
                                logger.log("OVERHEAD", f"agent:communication_conv_recv: {self.monitor.elapsed('agent:communication_conv_recv')}")
                                logger.log("OVERHEAD", f"agent:llm_backend_recv: {self.llm_monitor.elapsed('agent:llm_backend_recv')}")
                            else:
    ...
    def listen(self):
        ...
        context.options |= ssl.OP_NO_TLSv1 | ssl.OP_NO_TLSv1_1 | ssl.OP_NO_TLSv1_2  # TLS 1.3 only
        # For local dev, do not require client certificates
        context.verify_mode = ssl.CERT_NONE
        context.load_verify_locations(saga.config.CA_CERT_PATH)
        # Load the self-signed certificate and private key
        context.load_cert_chain(certfile=self.workdir + "agent.crt", keyfile=self.workdir + "agent.key")
