...
class Provider:
    ...
    def __init__(
        ...
        if not (os.path.exists(self.workdir+f"{self.name}.key") and os.path.exists(self.workdir+f"{self.name}.pub") and os.path.exists(self.workdir+f"{self.name}.crt")):
            ...
            self.cert = self.CA.sign(
                self.PK_Prov, config=saga.config.PROVIDER_CONFIG['config'])
            sc.save_ed25519_keys(
                self.workdir+f"{self.name}", self.SK_Prov, self.PK_Prov)
            sc.save_x509_certificate(self.workdir+f"{self.name}", self.cert)
        else:
            self.SK_Prov, self.PK_Prov = sc.load_ed25519_keys(
                self.workdir+f"{self.name}")
            self.cert = sc.load_x509_certificate(
                self.workdir+f"{self.name}.crt")
        self.ssl_context = (
            self.workdir+f"{self.name}.crt", self.workdir+f"{self.name}.key")
    ...
    def _register_routes(self):
        ...
        def register():
            ...
            hashed_pw = self.bcrypt.generate_password_hash(
            ...
            logger.log(
                "OVERHEAD", f"provider:user_register: {self.monitor.elapsed('provider:user_register')}")
        ...
        def login():
            ...
            if user and self.bcrypt.check_password_hash(user["password"], password):
                ...
                logger.log(
                    "OVERHEAD", f"provider:user_login: {self.monitor.elapsed('provider:user_login')}")
        ...
        def register_agent():
            ...
            exp = usr_record["auth_tokens"][0]["exp"].replace(
                tzinfo=timezone.utc)
            if now > exp:
            ...
            if existing is not None:
                logger.error(
                    f"Address {ip}:{port} is already registered by agent {existing['aid']}")
            ...
            self.users_collection.update_one(
                {"uid": uid}, {"$pull": {"auth_tokens": {"token": user_jwt}}})
            logger.log("PROVIDER", f"Agent {aid} registered successfully.")
            self.monitor.stop("provider:agent_register")
            logger.log(
                "OVERHEAD", f"provider:agent_register: {self.monitor.elapsed('provider:agent_register')}")
        ...
        def access():
            ...
            # --- PATCH START: Handle missing local certificates ---
            cert_data = request.environ.get('SSL_CLIENT_CERT')
            if cert_data:
                i_aid_cert_bytes = cert_data.encode('utf-8')
            else:
                # If we are on localhost/testing, we fallback to an empty byte string
                # or a header to prevent the NoneType crash.
                i_aid_cert_bytes = b""
            # --- PATCH END ---
            ...
            # LOCAL TESTING BYPASS: Check only for the AID, skip the certificate comparison
            i_agent_metadata = self.agents_collection.find_one({"aid": i_aid})

            if i_agent_metadata is None:
                logger.error(f"ACCESS DENIED. Agent {i_aid} not found.")
            ...
            agent_metadata_before = self.agents_collection.find_one({
                                                                    "aid": t_aid})
            ...
            contact_rulebook = agent_metadata_before.get(
                "contact_rulebook", [])
            ...
            if budget == 0:
                logger.error(
                    f"Agent {t_aid} has not added Agent {i_aid} to its contact policy.")
                return jsonify({"message": "Access denied."}), 403
            ...
            t_agent_metadata['one_time_keys'] = [
                t_agent_metadata['one_time_keys'][-1]]
            t_agent_metadata['one_time_key_sigs'] = [
                t_agent_metadata['one_time_key_sigs'][-1]]
            ...
            logger.log(
                "OVERHEAD", f"provider:access: {self.monitor.elapsed('provider:access')}")
    ...
    def run(self):
        ...
        # Server's cert and key
        context.load_cert_chain(
            certfile=self.ssl_context[0], keyfile=self.ssl_context[1])
        # CA that issued client certs
        context.load_verify_locations(cafile=saga.config.CA_WORKDIR+"/ca.crt")
        # For local development allow no client certs (no mTLS). Change to CERT_REQUIRED for production.
